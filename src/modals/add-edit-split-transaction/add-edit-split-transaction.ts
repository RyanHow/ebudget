...
export class AddEditSplitTransactionModal {
  ...
  constructor(private configuration: Configuration, private modalController: ModalController, public viewCtrl: ViewController, private navParams: NavParams, private engineFactory: EngineFactory, private nav: NavController, private alertController: AlertController) {
    this.budget = engineFactory.getEngineById(navParams.data.budgetId);
    this.budget.getCategory(navParams.data.categoryId);
    this.category = this.budget.db.transactionProcessor.table(Category).by('id', navParams.data.categoryId);
    ...
    if (navParams.data.transactionId) {
      this.editing = true;
      let transactionRecord = this.budget.db.transactionProcessor.table(Transaction).by('id', navParams.data.transactionId);
      this.transaction = this.budget.db.transactionProcessor.findTransactionsForRecord(transactionRecord, CreateSplitTransaction)[0];
  ...
  submit(event: Event) {
    ...
    this.budget.db.applyTransaction(t);
  ...
  deleteTransaction() {
    this.budget.db.deleteTransaction(this.transaction);
